...
**Purpose**: Systematically challenge every assumption and constraint,
classifying each as hard constraint (physics), soft constraint
(choice), or unvalidated assumption.
...
- For adversarial analysis (red-teaming an idea, penetration-test-style
...
Gather everything presented as a constraint — requirements, "we have
to..." statements, best practices, budget/timeline limits, policy —
then classify each:
...
- Every SOFT constraint traced to who decided it, why, and whether the
  original reason still holds; every ASSUMPTION paired with its
  evidence and a test that could disprove it.
- The **"remove it" test** applied to each non-hard constraint: if
  removing it unlocks significant value, it's worth challenging.
- The **hidden constraints** surfaced — the implicit ones so assumed
  they were never stated ("of course we need a database," "obviously
  this needs auth"). Those are the most dangerous.
...
4. **Approved Vendor List**: May exclude better solutions for no real
   risk reduction
...
**Adversarial review**: Use Challenge to attack the assumptions behind
any idea
```
→ Challenge on stated security controls
→ Challenge on business model assumptions
```

**Security testing**: Use Challenge to find real vs. assumed security
boundaries
```
→ Challenge on "the firewall protects us"
→ Challenge on trust boundaries
```

**Architecture review**: Use Challenge before accepting any requirement
```
→ Challenge on NFRs (non-functional requirements)
→ Challenge on technology choices
```
...
A run typically flows to:
- **Reconstruct** → Build solution using only hard constraints
- Or back to the requester with the constraint analysis for
  decision-making — Challenge's output is a complete deliverable on its
  own; nothing requires Reconstruct to follow.
