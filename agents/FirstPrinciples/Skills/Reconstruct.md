---
name: Reconstruct
agent: FirstPrinciples
---

# Reconstruct

**Purpose**: Build an optimal solution from scratch using only the
fundamental truths and hard constraints identified through Deconstruct
and Challenge.
...
> "If we knew nothing about how this is currently done, and only knew
> the fundamental truths, what would we build?"

This is about optimizing **function** (what you're trying to
accomplish) rather than **form** (how it's traditionally done).

---

## What a Done Reconstruction Shows

Build from only the hard constraints, designing as if you'd never seen
the current solution. A done Reconstruction exhibits:

- **Only the hard constraints stated** (physics, math, verified facts,
  true immutable requirements) — soft constraints, assumptions, and
  "how it's always been done" excluded.
- **Function defined as outcome, not method** — "persist and retrieve
  user data reliably," not "we need a database"; "deploy and scale
  components independently," not "we need microservices."
- **3+ blank-slate approaches** generated without filtering for
  feasibility, each checked against: satisfies the hard constraints,
  achieves the function, simpler than current. The best one maximizes
  simplicity.
- **Cross-domain synthesis** — an analogous problem solved in an
  unrelated field, applied here. (The snowmobile: tank treads + boat
  motor + bicycle seat = a new category.)
- **An explicit delta** vs the current approach: what gets eliminated
  (complexity that wasn't fundamental), what's new, what was actually
  correct all along.
...
- Approach: Single file database, blob storage, periodic backup to
  cloud
- Satisfies constraints: Yes - durable (with backups), addressable (by
  key), handles capacity
...
- Satisfies constraints: Yes - immutable = durable, hash = address,
  scales horizontally
...
Why: 90% of applications never exceed single-machine capacity. SQLite
is simpler, faster for reads, requires no infrastructure. Only
reconstruct to Option A/C when you actually hit limits.
...
- "We need a message queue" → Direct function calls work fine at our
  scale
- "We need Kubernetes" → A single server handles our load
- "We need a SPA framework" → Server-rendered HTML is simpler and
  faster
...
Removing soft constraints often allows combining what were separate
steps:
- "Microservices" → "Modular monolith" (one deployment, multiple
  modules)
...
**Full First Principles Flow** (typical, not required):
```
...
**Standalone Use**:
Reconstruct is independently invocable — use it directly if you already
know the hard constraints, without Deconstruct or Challenge having run:
```
→ Reconstruct given constraints [X, Y, Z]
```

**With Other Agents**:
- An architecture-review agent invokes this after Challenge to generate
  alternatives
- An engineering agent invokes this when stuck, to escape a local
  maximum
- An adversarial-review agent invokes this to construct counter-proposals
